...
This file contains codes for example "Six Cities" with t-distributied random effects:
1) GVA
2) SDGM
3) SDGM_Centered
...
from examples import init_SixCities,init_SixCities_t
import numpy as np
import scipy
import torch as t
import time
...
logp_0 = lambda th: t.tensor(EX0.logp(th.detach().numpy()))
gr_logp_0 = lambda th: t.tensor(EX0.gr_logp(th.detach().numpy())).flatten()
...
plt.figure(1)
ids = [0,1,2,537,538,539,540,541]
plt.clf()
name_list = [r"$\beta_1$",r"$\beta_2$",r"$\beta_3$",r"$\beta_4$",r"$\zeta$"]
...
plt.savefig(f"SixCities_m_t.pdf")
...
plt.savefig(f"SixCities_r_t.pdf")
#
...
plot_ELBO(VB_GVA.average_ELBOlog,VB_SDGM0.average_ELBOlog,VB_SDGM.average_ELBOlog,VB_SDGM_SAS.average_ELBOlog,VB_GVA_SAS.average_ELBOlog,
          ylim=[-1000, -250],subx=[430, 480],suby=[-317, -290],
          labels=['GVA','SDGM ','SDGM_C','SDGM+SAS','GVA+SAS'])
plt.savefig(f"SixCities_ELBO_t.pdf")
...
"""
VB_GVA_time = 80.9969892501831
VB_SDGM0_time = 99.11125111579895
VB_SDGM_time = 105.05633020401001
VB_SDGMSAS_time = 142.8800492286682
VB_GVASAS_time = 118.7606143951416
"""
