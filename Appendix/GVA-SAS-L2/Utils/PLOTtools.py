...
def plot_marginals(data, data2 =[], data3 = [], data4=[], data5=[],data6=[], data7=[], ids =[] ,num=[], bw = 'silverman', labels=[], plot_shape=[],fig_size=[]):
    plt.clf()
    ...
    for i in range(1,num+1):
        if plot_shape == []:
            ax = fig.add_subplot(int(np.ceil(np.sqrt(num))), int(np.ceil(np.sqrt(num))), i)
        else:
        ...
        if data6 != []:
            ...
            plt.plot(x, y, 'brown', label='KDE /w ISJ')

        if data7 != []:
            x, y = FFTKDE(kernel='gaussian', bw=bw).fit(data7[:,ids[i-1]]).evaluate()
            plt.plot(x, y, 'black', label='KDE /w ISJ')

        plt.tight_layout()
               

def plot_randeff(data_mcmc, data_vb, data_vb2 = [], data_vb3=[], data_vb4=[], data_vb5=[], data_vb6=[], mean=True, sd=False, ids =[] ,num=[], bw = 'silverman', labels=[]):
    ...
    if mean:
        ...
        if data_vb6 != []:
            ax = fig.add_subplot(len(labels), 3, 16)
            ax.plot(np.mean(data_mcmc, 0), np.mean(data_vb6, 0), 'o',alpha=0.2)
            lims = [
                np.min([ax.get_xlim(), ax.get_ylim()]),  # min of both axes
                np.max([ax.get_xlim(), ax.get_ylim()]),  # max of both axes
            ]
            ax.plot(lims, lims, 'r-', c='tomato', zorder=0)
            if len(labels) > 5: ax.set_ylabel(labels[5])
            if len(labels) == 6: ax.set_xlabel("MCMC")
            ax = fig.add_subplot(len(labels), 3, 17)
            x_std, y_std = np.std(data_mcmc, 0), np.std(data_vb6, 0)
            ax.plot(x_std, y_std, 'o',alpha=0.2)
            lims = [
                np.min([ax.get_xlim(), ax.get_ylim()]),  # min of both axes
                np.max([ax.get_xlim(), ax.get_ylim()]),  # max of both axes
            ...
            ax.plot(lims, lims, 'r-', c='tomato',zorder=0)
            if len(labels) == 6: ax.set_xlabel("MCMC")
            ax = fig.add_subplot(len(labels), 3, 18)
            x_skewness, y_skewness = scipy.stats.skew(data_mcmc, 0), scipy.stats.skew(data_vb6, 0)
            ax.plot(x_skewness, y_skewness, 'o',alpha=0.2)
            lims = [
                np.min([ax.get_xlim(), ax.get_ylim()]),  # min of both axes
                np.max([ax.get_xlim(), ax.get_ylim()]),  # max of both axes
...
def plot_ELBO(ELBO1,ELBO2=[],ELBO3=[],ELBO4=[],ELBO5=[],ELBO6=[],ylim=[],subx=[],suby=[],labels=[]):
    plt.clf()
    plt.figure()
    ...
    if ELBO6 != []:
        axins.plot(ELBO6, 'brown',alpha=0.8)
    # 调整子坐标系的显示范围
    if subx != []:
