...
def init_SixCities_simulation(n,r):
    root = "".join([os.getcwd(), "/Examples/Linear Mixed Effects/SixCities/"])
    STANfile = "".join([root, "model_wheeze.stan"])
    name_dat = f"wheeze-{n}-{r}.rds"
    datfile = "".join([root, name_dat])
    data = rds2dict(datfile)
    EX = Stan2Py(STANfile, data, name=f"SixCities-{n}-{r}")

    Tmask = np.eye(n+5)
    Tmask[n:, :] = np.ones((5, n+5))
    EX.Tmask = torch.tensor(np.tril(Tmask))
    # name_mcmc = f"wheeze{n}-{r}-mcmc.rds"
    # datfile_moments = "".join([root, "wheeze100_mcmc.rds"])
    # EX.mcmc_moments = rds2dict(datfile_moments)

    return EX


def import_mcmc_result(n,R):
    root = "".join([os.getcwd(), "/Examples/Linear Mixed Effects/SixCities/"])
    name_mcmc = f"wheeze-{n}-{R}-mcmc.rds"
    datfile_moments = "".join([root, name_mcmc])
    mcmc_moments = rds2dict(datfile_moments)

    return mcmc_moments

def import_test_data(n,r):
    root = "".join([os.getcwd(), "/Examples/Linear Mixed Effects/SixCities/"])
    name_dat = f"wheeze-test-{n}-{r}.rds"
    datfile_moments = "".join([root, name_dat])
    test_dat = rds2dict(datfile_moments)
